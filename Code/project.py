...
tweets = []
count=0
y=[2, 4, 4, 4, 5, 2, 3, 4, 3, 3, 1, 3, 3]
for line in open('train.txt').readlines():
    #items = line.split(',')
    tweets.append([y[count], line.lower().strip()])
    count=count+1
...
with open('predicted_tweets.txt','w') as write_file:
    for id in range(5): #nuber of movies in test data
        write_file.write(str(y[id])+","+str(tweets[id]))
